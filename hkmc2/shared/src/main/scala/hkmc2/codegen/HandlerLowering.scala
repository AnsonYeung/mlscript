    ...
    stackSafetyMap += ctx.resumeInfo.currentStackSafetySym ->
      (
        ...
        locally:
          ...
          r(ctx.unwindCall(ctx.resumeInfo.currentStackSafetySym.fold(_.toLoc, _.toLoc).fold(unit)(locToStr(_)), if ExceptionToggle then Value.SimpleRef(pcVar) else intLit(-1), Nil)(using paths))
      )
