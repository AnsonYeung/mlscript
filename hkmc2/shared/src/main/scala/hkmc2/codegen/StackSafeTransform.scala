        ...
        val inner = blk match
          case s @ Scoped(body = m @ Match(rest = tc @ TryCatch(sub = tryBlk))) =>
            s.copy(body = m.copy(rest = tc.copy(sub =
              blockBuilder
                .assign(State.noSymbol, Call(checkDepthPath, Nil ne_:: Nil)(CallMetadata.mlsFunWithEffect))
          ...
          case _ =>
            ...
            TryCatch(
              ...
              tmp,
              Assign(State.noSymbol, Call(paths.runtimePath.selSN("effectRethrow"), (tmp.asPath.asArg :: Nil) ne_:: Nil)(CallMetadata.mlsFunWithEffect), doUnwindBlk),
              End(),
            )
        blockBuilder
          .assignFieldN(runtimePath, STACK_DEPTH_IDENT, op("+", stackDepthPath, intLit(increment)))
